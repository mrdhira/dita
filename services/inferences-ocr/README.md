...
all live in [`packages/pylibs/worker`](../../packages/pylibs/worker); the wire is
...
Everything that is not OCR lives in [`packages/pylibs/worker`](../../packages/pylibs/worker):
...
`worker`, a workspace source rather than a version — and resolved in the single
...
26 tests here, and 80 in [`packages/pylibs/worker`](../../packages/pylibs/worker/README.md#tests)
...
4. If the engine is not `rapidocr`, `tesseract` or `manga_ocr`, add an adapter under
   `ocr_worker/engines/`: subclass `worker.Engine`, implement `infer`, then add the
