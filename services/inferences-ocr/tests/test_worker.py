...
manifest they are selected by, and the identity this service hands `worker`.

The framework -- the registry parser, the fetcher, the manager, the socket server, the
health probes and the framing -- is tested in `packages/pylibs/worker`, because that
...
from worker import ModelManager, Result, UnknownEngine, dispatch, load_registry
...
class ServiceIdentityTest(unittest.TestCase):
    """The wiring: what this service tells `worker` about itself.
    ...
    def test_the_module_entrypoint_hands_this_worker_to_the_framework(self) -> None:
        ...
        with mock.patch("logging.basicConfig"), self.assertLogs("worker.cli", "ERROR"):
